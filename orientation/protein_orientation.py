...
from calc_angles import get_com, get_principal_axes, make_direction_cosine_matrix
...
def run_multiple(
    ...
    for i, ts in enumerate(u.trajectory[::skip_val]):
        ...
        user_ax1 = list(
            u.select_atoms(
                "resid " + str(user_resid_pitch_sel) + " and name CA"
            ).atoms.positions[0]
            - u.select_atoms(sel).center_of_mass()
        )

        # Define the initial roll vector based on the users choice
        user_ax2 = list(
            u.select_atoms(
                "resid " + str(user_resid_roll_sel) + " and name CA"
            ).atoms.positions[0]
            - u.select_atoms(sel).center_of_mass()
        )
        ...
        user_ax3 = list(
            u.select_atoms(
                "resid " + str(user_resid_yaw_sel) + " and name CA"
            ).atoms.positions[0]
            - u.select_atoms(sel).center_of_mass()
        )
        ...
        if calc_method == "user_pa":
            ...
            dir_cosine_matrix = make_direction_cosine_matrix(
                ref=ref, axes_set=pa_array_row_wise
            )
            ...
            yaw, pitch, roll = (
                np.round(euler_angles_array[0], 2),
                np.round(euler_angles_array[1], 2),
                np.around(euler_angles_array[2], 2),
            )
...
def init_parser():
    ...
    parser.add_argument(
        ...
        default=["strand", "310helix", "alphahelix"],
...
if __name__ == "__main__":
    ...
    prot_sel_length = len(
        temp_universe.select_atoms(
            "resid "
            + str(start_of_protein)
            + ":"
            + str(end_of_protein)
            + " and name CA"
        )
    ...
    protein_info = read_stride(
        stride_file=options.stride_file,
    ...
    if options.pa_single:  # If the user just wants to find the principal axes
        ...
        results = Parallel(n_jobs=options.nprocs, verbose=3, backend="multiprocessing")(
